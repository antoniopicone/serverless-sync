...
write() { curl -sS -XPOST "$1/write" -H 'content-type: application/json' \
               -d "{\"entity\":\"$2\",\"value\":\"$3\"}" >/dev/null; }
state() { curl -sS "$1/state"; }

# fingerprint <base-url> — straight from /v1/node, no logger involved:
# this is what phase 5 uses to prove convergence keeps happening with the
# observer turned off.
fingerprint() {
  curl -sS "$1/v1/node" | python3 -c 'import sys,json;print(json.load(sys.stdin).get("fingerprint",""))'
}
